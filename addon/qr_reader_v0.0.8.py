...
# ==========================================
# 3. 설치가 보장된 상태에서 비로소 패키지들을 불러옴 (OS 예외 처리 포함)
# ==========================================
...
try:
    ...
    # 맥OS에서 pip 설치는 되었으나 시스템 레벨의 zbar 부품이 없을 때 발생하는 에러 방어
...
# --- 윈도우 화면 배율(DPI) 강제 인식 (맥에서는 자동으로 무시됨) ---
try:
...
def update_schedule_listbox():
    """scan_schedules와 last_scanned_info 데이터를 조합하여 리스트박스를 실시간으로 다시 그립니다."""
...
# 프로그램 우측 하단 버전 표시 (v0.0.8 적용)
version_label = tk.Label(root, text="v0.0.8", font=("Malgun Gothic", 8), fg="gray")
version_label.pack(side="bottom", anchor="e", padx=10, pady=5)

# --- OS별 단축키 동적 바인딩 (크로스 플랫폼 대응) ---
root.bind('<Control-v>', load_from_clipboard)  # 윈도우/리눅스용
if sys.platform == 'darwin':
    root.bind('<Command-v>', load_from_clipboard)  # 맥OS용
