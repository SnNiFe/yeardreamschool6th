...
SELECT
    ...
    , InvoiceDate
    , Total
    , ROUND(SUM(Total) OVER(), 2) AS GrandTotal -- 윈도우 함수 문법
    , SUM(Total) OVER(
...
SELECT 
    ...
    , Title
    , HireDate
    , NTILE(3) OVER(ORDER BY HireDate ASC) AS HireGroup
FROM employees
...
SELECT 
    ...
    , COUNT(t.TrackId)           AS TrackCount -- 트랙의 개수
    , ROUND(AVG(t.UnitPrice), 2) AS AvgPrice -- 평균 단가
FROM tracks t
...
GROUP BY g.Name, mt.Name          -- (장르, 미디어타입) 조합
