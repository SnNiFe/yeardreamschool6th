...
SELECT productName, buyPrice
    , ROW_NUMBER() OVER(ORDER BY buyPrice DESC) AS row_num
FROM products
...
SELECT customerName, country, creditLimit
    , AVG(creditLimit) OVER(PARTITION BY country) AS avgCreditByCountry
    -- , creditLimit - AVG(creditLimit) OVER(PARTITION BY country) AS 증감
FROM customers
WHERE creditLimit IS NOT NULL -- NULL을 제외한 값을 조회한다.
...
SELECT c.customerNumber, c.customerName
    ...
    -- 확장해서 생각
    -- 궁금증? 시분초? ==> 공식문서에 답이 이미 존재함 https://sqlite.org/windowfunctions.html
    , strftime('%Y-%m', paymentDate) AS payYearMonth
    , AVG(p.amount) AS avgMonthAmount
    , RANK() OVER(PARTITION BY strftime('%Y-%m', paymentDate) ORDER BY AVG(p.amount) DESC) AS monthAvgRank
FROM customers c
INNER JOIN payments p ON p.customerNumber = c.customerNumber
GROUP BY payYearMonth, c.customerNumber
) t
WHERE
    t.monthAvgRank = 1
ORDER BY
...
SELECT customerNumber, paymentDate, amount
    , SUM(amount) OVER(ORDER BY paymentDate ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS runningTotal
FROM payments
...
SELECT customerNumber, paymentDate, amount
    , LAG(amount, 1) OVER(PARTITION BY customerNumber ORDER BY paymentDate ASC) AS prevAmount
FROM payments
...
--   - nextAmount     : 다음 결제 금액
SELECT customerNumber, paymentDate, amount
    , LEAD(amount, 1) OVER(PARTITION BY customerNumber ORDER BY paymentDate ASC) AS nextAmount
FROM payments
ORDER BY customerNumber ASC, paymentDate ASC
LIMIT 15
;
...
SELECT DISTINCT country
    , FIRST_VALUE(customerName) OVER(PARTITION BY country ORDER BY paymentDate DESC) AS firstCustomer
    , FIRST_VALUE(paymentDate) OVER(PARTITION BY country ORDER BY paymentDate DESC) AS firstPaymentDate
FROM (
SELECT c.country
    , c.customerName
    , p.paymentDate
...
--SELECT NTILE(4) OVER(PARTITION BY paymentDate ORDER BY year_total)
--FROM (
--SELECT customerNumber, paymentDate, amount
--    , SUM(amount) OVER(PARTITION BY customerNumber) AS cust_total
--    , SUM(amount) OVER(PARTITION BY paymentDate) AS year_total
--FROM payments
--);
...
--    → 인라인 뷰(cust_pay)에서 처리 ==> 첫번쨰 인라인 뷰
...
---- 2, 3번 쿼리를 묶어서 처리하고 => 인라인 뷰로 다시 정리
...
;
--ORDER BY
--    country, countryRank ASC;                -- 국가오름차순, 국가내 순위 오름차순
