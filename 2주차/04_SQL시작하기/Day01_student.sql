...
SELECT * 
FROM customers 
WHERE Country != 'USA';     -- WHERE NOT Country = 'USA';
...
SELECT * 
FROM customers 
WHERE Country IN ('USA', 'Canada', 'Brazil'); -- 위와 동일
 
-- 문제 3.
-- 위 세 나라를 "제외한" 나머지 나라 고객을 보려 합니다.
SELECT * 
FROM customers 
WHERE NOT Country IN ('USA', 'Canada', 'Brazil');
...
SELECT Name, UnitPrice
FROM tracks
ORDER BY 1 DESC; -- 1은 Name > 이름 내림차순 정렬
--
SELECT Name AS "앨범이름", UnitPrice -- Name 과 "앨범이름" 둘 다 사용 가능
FROM tracks
ORDER BY "앨범이름" DESC;
-- 각 DBMS 마다 문법이 조금씩 다를 수 있다!
